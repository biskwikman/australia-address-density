  ...
  function setMap(){
    ...
    d3.queue()
              .defer(d3.json, "data/post2012aus.json")//load geometry
              .await(callback); //trigger callback function once data is loaded
              
    function callback(error, ausData){
        console.log();
        var recolorMap = colorScale(csvData);
                      
        //variables for csv to json data transfer
        var jsonLgas = ausData.objects.prefectures.geometries;
        ...
        for (var i=0; i<csvData.length; i++) {
          ...
          for (var a=0; a<48; a++) {
            ...
            if (jsonLgas[a].properties.id == csvId) {
              //assign key/value pair
              
              for (var key in keyArray) {
                var attr = keyArray[key];
                var val = parseFloat(csvPrefecture[attr]);
                jsonLgas[a].properties[attr] = val;
              };
        ...
        var prefectures = map.selectAll(".prefectures")
                              .data(topojson.feature(ausData, ausData.objects.prefectures).features)
                            .enter().append("path")
